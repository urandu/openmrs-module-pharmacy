...
<c:choose>
    <c:when test="${not empty jsVar}">
        ...
        var ${jsVar} = new Array();
        <c:if test="${optionHeader != ''}"><c:if test="${optionHeader == '[blank]'}"> ${jsVar}.push({ val: '', display:
            '' }); </c:if>
            <c:if test="${optionHeader != '[blank]'}"> ${jsVar}.push({ val: '', display: '${optionHeader}'
                }); </c:if></c:if>
        <c:forEach items="${reasons}" var="reason"> ${jsVar}.push({ val: '${reason.key}', display: '${reason.value}'
            }); </c:forEach>
    ...
    <c:otherwise>
        ...
        <c:forEach items="${reasons}" var="reason">
            ...
            <c:if test="${reason.key == initialValue}">selected</c:if>>${reason.value}</option>
        </c:forEach>
        </select>
    </c:otherwise>
</c:choose>