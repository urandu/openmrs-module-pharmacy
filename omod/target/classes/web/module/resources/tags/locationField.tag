...
<c:choose>
    <c:when test="${locationFieldStyle == 'tree'}">
        <openmrs_tag:locationTree formFieldName="${formFieldName}" initialValue="${initialValue}"
                                  selectableTags="${selectableTags}"/>
    </c:when>
    <c:otherwise>
        <select name="${formFieldName}" id="${formFieldName}"<c:if test="${not empty onChange}">
        onChange=${onChange}</c:if>>
        ...
        <openmrs:forEachRecord name="locationHierarchy">
            <option value="${record.location.locationId}"
            <c:if test="${record.location == initialValue}">selected</c:if>>
