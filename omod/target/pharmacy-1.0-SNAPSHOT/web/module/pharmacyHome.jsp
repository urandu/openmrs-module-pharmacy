...
<c:when test="${model.size == 'slowConnection'}">
    ...
    <script>
        ...
        function findObjects(text) {
            ...
            :
                message
                code = "error.patientSearchNoChars" / >
            )
                ;
...
<c:when test="${model.size == 'full'}">
    ...
    <script type="text/javascript">
        ...
        $j(document).ready(function () {
            ...
            var =
            "attribute"
            items = "${fn:split(attributesToList, ',')}"
            varStatus = "varStatus" >
                    < c
            :
            if test = "${fn:trim(attribute) != ''}" >
                    < c:
            set
            var =
            "attributeName"
            value = "${fn:trim(attribute)}" / >
                    < c
            :
            choose >
            < c
            :
            when
            test = "${varStatus.index == 0}" >
                    {name: "${attributeName}", header: "<openmrs:message code="PersonAttributeType.
            ${fn:replace(attributeName, ' ', '')}" text="
            ${attributeName}"/>"
        }
                <
        /
        ...
        c:if test = "${not empty param.phrase}" >
                , searchPhrase:
        '<openmrs:message text="${ param.phrase }" javaScriptEscape="true"/>'
        <
        /
        c:if>
        })
