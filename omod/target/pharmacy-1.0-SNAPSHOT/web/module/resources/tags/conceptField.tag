...
<%-- a concept id to show answers for --%>
...
<%-- Gets the full ConceptListItem object --%>
...
<script type="text/javascript">
    ...
    $j(document).ready(function () {
        ...
        :
        when
        test = "${not empty includeClasses}" > "${includeClasses}".split(",") < /c:when> <c:otherwise> null </
        ...
        :
        when
        test = "${not empty excludeClasses}" > "${excludeClasses}".split(",") < /c:when> <c:otherwise> null </
        c:otherwise > </c:choose >;
        var includeD =
        <
        c:choose > < c
        :
        when
        test = "${not empty includeDatatypes}" > "${includeDatatypes}".split(",") < /c:when> <c:otherwise> null </
        c:otherwise > </c:choose >;
        var excludeD =
        <
        c:choose > < c
        :
        when
        test = "${not empty excludeDatatypes}" > "${excludeDatatypes}".split(",") < /c:when> <c:otherwise> null </
        c:otherwise > </c:choose >;
        ...
        c:if test = "${not empty initialValue}" >
            // fetch the concept object they passed the value in of and do the normal "select" stuff
                DWRConceptService.getConcept("${initialValue}", function (concept) {
                    func${escapedFormFieldId}AutoCompleteOnSelect(concept);
                });
        </c:if>
