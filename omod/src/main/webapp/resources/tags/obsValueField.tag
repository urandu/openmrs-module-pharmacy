    ...
    <c:choose>
        <c:when test="${c.datatype.boolean}">
			<span id="valueBooleanRow${conceptId}" class="obsValue">
				<select name="${formFieldName}" id="valueBooleanField${conceptId}">
                    <option value=""
                    <c:if test="${initialValue == null}">selected</c:if>></option>
                    <option value="1"
                    <c:if test="${initialValue != null && initialValue != 0}">selected</c:if>><openmrs:message
                        code="general.true"/></option>
                    <option value="0"
                    <c:if test="${initialValue == 0}">selected</c:if>><openmrs:message code="general.false"/></option>
                </select>
			</span>
        </c:when>
        <c:when test="${c.datatype.text}">
			<span id="valueTextRow${conceptId}" class="obsValue">
				<textarea name="${formFieldName}" id="valueTextField${conceptId}" rows="${empty rows ? 3 : rows}"
                          cols="${empty cols ? 35 : cols}">${initialValue}</textarea>
			</span>
        </c:when>
        <c:when test="${c.datatype.numeric}">
			<span id="valueNumericRow${conceptId}" class="obsValue">
				<input type="text" name="${formFieldName}" id="valueNumericField${conceptId}" value="${initialValue}"
                       size="${empty size ? 10 : size}" onblur="validateNumericRange('${c.conceptId}', this)"/>
				<c:if test="${empty showUnits || showUnits=='true'}"><span
                        id="numericUnits${conceptId}">${num.units}</span></c:if>
				<span id="numericRangeError${c.conceptId}"></span>
			</span>
        </c:when>
        <c:when test="${c.datatype.date}">
			<span id="valueDateRow${conceptId}" class="obsValue">
				<input type="text" name="${formFieldName}" id="valueDateField${conceptId}" value="${initialValue}"
                       size="${empty size ? 10 : size}" onFocus="showCalendar(this)"/>
        ...
        <c:when test="${c.datatype.time}">
			<span id="valueTimeRow${conceptId}" class="obsValue">
				<input type="text" name="${formFieldName}" id="valueTimeField${conceptId}" value="${initialValue}"
                       size="${empty size ? 10 : size}" onFocus="showTimePicker(this)"/>
        ...
        <c:when test="${c.datatype.dateTime}">
			<span id="valueDatetimeRow${conceptId}" class="obsValue">
				<input type="text" name="${formFieldName}" id="valueDatetimeField${conceptId}" value="${initialValue}"
                       size="${empty size ? 15 : size}" onFocus="showDateTimePicker(this)"/>
				<c:if test="${empty showFormat || showFormat=='true'}">(<openmrs:datePattern/> <openmrs:timePattern
                        format="jquery"/>)</c:if>
			</span>
        </c:when>
			...
			<span id="valueCodedRow${conceptId}" class="obsValue">
				<c:choose>
                    <c:when test="${!empty c.answers}">
                        <openmrs_tag:conceptAnswerField concept="${c}" formFieldName="${formFieldName}"
                                                        initialValue="${initialValue}"/>
                    </c:when>
                    <c:when test="${empty c.answers && c.set}">
                        <openmrs_tag:conceptSetField conceptId="${conceptId}" formFieldName="${formFieldName}"
                                                     initialValue="${initialValue}"/>
                    </c:when>
                    <c:otherwise>
