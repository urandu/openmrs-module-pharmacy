...
<c:choose>
    <c:when test="${err == null}">
        <input type="text" id="${formFieldId != null ? formFieldId : formFieldName}" name="${formFieldName}" size="10"
               value="<openmrs:formatDate date="${startValue}" />" onFocus="showCalendar(this)" /><span
            class="datePatternHint"> (<openmrs:datePattern/>)</span>
    </c:when>
    <c:otherwise>
        <input type="text" id="${formFieldId != null ? formFieldId : formFieldName}" name="${formFieldName}" size="10"
               value="${startValue}" onFocus="showCalendar(this)"/><span
            class="datePatternHint"> (<openmrs:datePattern/>)</span>
